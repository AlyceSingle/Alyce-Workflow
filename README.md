...
如果你的 SillyTavern 版本支持第三方扩展安装器，可以在扩展安装器中填入：
```
https://github.com/AlyceSingle/Alyce-Workflow
```
安装完成后刷新页面，再从“扩展”面板打开 `Alyce 工作台`。
...
- 删除：清除单条记录
- 清除全部：清空当前事件流
- 继续：工作流中断后，从失败的环节与轮次继续，保留已经完成的资产
- 重新开始：工作流中断后，使用同一条用户输入从第一个启用环节重新执行
...
当某个环节开启 `启用编辑工具` 后，这个环节的输出不会被当成普通正文保存，而会被当成 EDIT 指令解析。
